...
const CollageCreator = () => {
  ...
  const fetchAlbumData = async () => {
    try {
      const token = localStorage.getItem("token")
      // נסה קודם השרת המקומי, ואז Render
      let response
      try {
        response = await axios.get(`https://localhost:7251/api/album/${albumId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } catch (localError) {
        console.log("נכשל בחיבור לשרת המקומי, מנסה Render...")
        response = await axios.get(`https://memoria-api-pukg.onrender.com/api/album/${albumId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  // פונקציה משופרת לטעינת תמונה עם מספר אסטרטגיות
  const loadImageViaProxy = async (imageUrl: string): Promise<HTMLImageElement> => {
    return new Promise(async (resolve, reject) => {
      const token = localStorage.getItem("token")

      // אסטרטגיה 1: נסה לטעון ישירות מ-S3
      const tryDirectLoad = () => {
        return new Promise<HTMLImageElement>((resolveDirectly, rejectDirectly) => {
          const image = new Image()
          image.crossOrigin = "anonymous"
          image.onload = () => {
            console.log(`✅ תמונה נטענה ישירות מ-S3: ${imageUrl}`)
            resolveDirectly(image)
          }
          image.onerror = () => {
            console.log(`❌ נכשל בטעינה ישירה מ-S3: ${imageUrl}`)
            rejectDirectly(new Error("Direct load failed"))
          }
          image.src = imageUrl
        })
      }

      // אסטרטגיה 2: נסה דרך השרת המקומי
      const tryLocalProxy = async () => {
        try {
          console.log(`🔄 מנסה לטעון דרך השרת המקומי: ${imageUrl}`)
          ...
          return new Promise<HTMLImageElement>((resolveLocal, rejectLocal) => {
            const image = new Image()
            image.crossOrigin = "anonymous"
            image.onload = () => {
              URL.revokeObjectURL(blobUrl)
              console.log(`✅ תמונה נטענה דרך השרת המקומי: ${imageUrl}`)
              resolveLocal(image)
            }
            image.onerror = () => {
              URL.revokeObjectURL(blobUrl)
              rejectLocal(new Error("Local proxy load failed"))
            }
            image.src = blobUrl
          })
        } catch (error) {
          console.log(`❌ נכשל בטעינה דרך השרת המקומי: ${(error as Error).message}`)
          throw error
        }
      }

      // אסטרטגיה 3: נסה דרך השרת על Render (רק אם השרת המקומי לא זמין)
      const tryRenderProxy = async () => {
        try {
          console.log(`🔄 מנסה לטעון דרך Render: ${imageUrl}`)
          const response = await axios.get(`https://memoria-api-pukg.onrender.com/api/image/proxy`, {
            params: { url: imageUrl },
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          return new Promise<HTMLImageElement>((resolveRender, rejectRender) => {
            const image = new Image()
            image.crossOrigin = "anonymous"
            image.onload = () => {
              URL.revokeObjectURL(blobUrl)
              console.log(`✅ תמונה נטענה דרך Render: ${imageUrl}`)
              resolveRender(image)
            }
            image.onerror = () => {
              URL.revokeObjectURL(blobUrl)
              rejectRender(new Error("Render proxy load failed"))
            }
            image.src = blobUrl
          })
        } catch (error) {
          console.log(`❌ נכשל בטעינה דרך Render: ${(error as Error).message}`)
          throw error
        }
      }

      // נסה את האסטרטגיות לפי סדר עדיפות
      try {
        // קודם נסה טעינה ישירות
        const image = await tryDirectLoad()
        resolve(image)
      } catch (directError) {
        try {
          // אם נכשל, נסה דרך השרת המקומי
          const image = await tryLocalProxy()
          resolve(image)
        } catch (localError) {
          try {
            // אם גם זה נכשל, נסה דרך Render (למקרה שהבעיה נפתרה)
            const image = await tryRenderProxy()
            resolve(image)
          } catch (renderError) {
            console.error(`❌ כל האסטרטגיות נכשלו עבור: ${imageUrl}`)
            reject(new Error(`Failed to load image with all strategies: ${imageUrl}`))
          }
        }
      }
    })
  ...
  const createCollage = async (selectedImages: Image[], customSettings?: any, customBgType?: string) => {
    ...
    try {
      ...
      if (loadedImages.length === 0) {
        // הצג הודעה מפורטת יותר למשתמש
        const errorMessage =
          failedImages.length > 0
            ? `לא ניתן לטעון אף תמונה. תמונות שנכשלו: ${failedImages.join(", ")}\n\nייתכן שיש בעיית רשת או חסימה של NetFree.`
            : "לא ניתן לטעון אף תמונה"
      ...
      if (failedImages.length > 0) {
        console.warn(
          `נטענו ${loadedImages.length} תמונות מתוך ${selectedImages.length}. תמונות שנכשלו: ${failedImages.join(", ")}`,
        )
      } else {
        console.log(`נטענו כל ${loadedImages.length} התמונות בהצלחה!`)
      }
